...
func (l *Log) tryMigrate() {
	...
	if err != nil {
		l.Warn(fmt.Sprintf("Couldn't migrate file: %s", err), LogGroup.AUDIT)
		return
	}
	...
	if fileErr != nil {
		l.Warn(fmt.Sprintf("Error while closing file: %s", fileErr), LogGroup.AUDIT)
	}
}
...
func (l *Log) enqueue(callstack []byte, group string, s any, otherGroups ...string) *ErrorPromise {
	...
	if err := l.queue.Push(entry); err != nil {
		log.Panic("enqueue called on closed queue")
	}
	return promise
}
...
func (l *Log) Panic(s any, groups ...string) {
	l.enqueue(debug.Stack(), LogGroup.PANIC, s, groups...).Await() //nolint
}
...
func (l *Log) Close() error {
	...
	if l.awaitConsumerOrTimeout(10000) {
		return errors.New("logger timed out; messages may be lost")
	}
	l.queue.Close()
	return l.file.Close()
}
